...
cat tmp445649 | awk -v BoxX=$BoxX -v BoxY=$BoxY -v BoxZ=$BoxZ -f ./sign.awk > tmp445650
...
#~ cat $filein | awk '{if( $1<0 && (($1)*(-1))>'$BoxSize/2') {x=$1+'$BoxSize';} if( $1>0 && $1>'$BoxSize/2') {x=$1-'$BoxSize';} print x;}' > input1.txt
#~ cat $filein | awk '{if( $2<0 && (($2)*(-1))>'$BoxSize/2') {y=$2+'$BoxSize';} if( $2>0 && $2>'$BoxSize/2') {y=$2-'$BoxSize';} print y;}' > input2.txt
#~ cat $filein | awk '{if( $3<0 && (($3)*(-1))>'$BoxSize/2') {z=$3+'$BoxSize';} if( $3>0 && $3>'$BoxSize/2') {z=$3-'$BoxSize';} print z;}' > input3.txt
#~ printf "" > input4.txt
#~ 
#~ for i in 1 $Nframes
#~ do 
#~ a=$(cat input1.txt | sed -n "$i"p | awk '{printf "%f",$1}')
#~ b=$(cat input2.txt | sed -n "$i"p | awk '{printf "%f",$1}')
#~ c=$(cat input3.txt | sed -n "$i"p | awk '{printf "%f",$1}')
#~ printf "%f %f %f \n" $a $b $c >> input4.txt
#~ done
 
#cat $file1 | awk '{if( $4>20 && $1<60 ) {p=($3-1)*60*16+($2-1)*60+$1; ; print p" "q" "  )" "$4" [" $1" "$2" "$3" "$4" +x]"}}' > input.tmp
#A=-10
#B=${A#-}
